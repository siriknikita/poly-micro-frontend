...
function StatusBadge({
  ...
  if (
    CLASSES_BY_SEVERITY[
      status.toLowerCase() as keyof typeof CLASSES_BY_SEVERITY
    ]
  ) {
    styleClass =
      CLASSES_BY_SEVERITY[
  ...
  // Then try to match with service statuses (Running, Healthy, etc.)
    ...
    SERVICE_STATUS_CLASSES[status as keyof typeof SERVICE_STATUS_CLASSES]
  ) {
    styleClass =
      SERVICE_STATUS_CLASSES[status as keyof typeof SERVICE_STATUS_CLASSES];
  }
  // For case-insensitive matching with service statuses
    ...
    if (statusKey) {
      styleClass =
        SERVICE_STATUS_CLASSES[
          statusKey as keyof typeof SERVICE_STATUS_CLASSES
        ];
    } else {
